"""
ADB Project Description Extractor
...
INPUT_CSV = DATA_DIR / "Final ADB Corpus - Sheet1_with_urls.csv"
OUTPUT_CSV = DATA_DIR / "Final ADB Corpus - Sheet1_with_urls.csv"
TEST_SIZE = 5
...
async def load_csv_data():
    ...
    # Find header row (by "Project ID" in first column or "Project Link" anywhere)
    def is_header(r):
        if len(r) == 0:
            return False
        if r[0] == "Project ID":
            return True
        return "Project Link" in r

    header_idx = next(i for i, r in enumerate(rows) if is_header(r))
...
def save_results(rows, output_file):
    ...
    with open(str(output_file), 'w', encoding='utf-8', newline='') as f:
